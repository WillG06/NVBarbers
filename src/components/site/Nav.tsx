  ...
  if (mobile) {
    return (
      <Link
        ...
        style={{ border: "1px solid var(--leather)", borderRadius: "999px", color: "var(--leather)", touchAction: "manipulation", WebkitTapHighlightColor: "transparent" }}
      >
        Reserve a chair →
      </Link>
    );
  ...
  return (
    <Link
      ...
      style={{
        ...
        background: hovered ? "var(--leather)" : "transparent",
        transition: "all 0.45s cubic-bezier(0.22,1,0.36,1)",
        letterSpacing: "0.26em",
        fontSize: "10.5px",
        textTransform: "uppercase",
        boxShadow: hovered ? "0 0 24px color-mix(in oklab, var(--leather) 28%, transparent)" : "0 0 0px transparent",
    ...
    >
      <span style={{ position: "relative", zIndex: 1, fontWeight: 500 }}>Reserve</span>
      <span aria-hidden style={{ position: "relative", zIndex: 1, display: "inline-block", transition: "transform 0.35s cubic-bezier(0.22,1,0.36,1)", transform: hovered ? "translateX(4px)" : "translateX(0)" }}>→</span>
    </Link>
  );
}
...
export function Nav() {
  ...
  return (
    <header
      ...
      style={{
        /*
         * top: 0 — background starts at the absolute top of the viewport,
         * filling behind the Dynamic Island / status bar.
         *
         * paddingTop: env(safe-area-inset-top) — nav CONTENT is pushed below
         * the notch.  The background colour fills the space above it.
         *
         * On non-notched devices env() resolves to 0, no effect.
         */
        top: 0,
        paddingTop: "env(safe-area-inset-top)",
        background: scrolled
          ? "color-mix(in oklab, var(--foreground) 91%, transparent)"
          : "linear-gradient(180deg, rgba(8,14,14,0.52) 0%, rgba(8,14,14,0.20) 65%, rgba(8,14,14,0.0) 100%)",
        backdropFilter: scrolled ? "blur(20px) saturate(130%)" : "none",
        borderBottom: scrolled ? "1px solid color-mix(in oklab, var(--leather) 16%, transparent)" : "none",
    ...
    >
      {/*
        ...
        regardless of device.  Position absolute, top = safe-area-inset-top.
      */}
      <div
        aria-hidden
      ...
      <div className="mx-auto flex max-w-[1600px] items-center justify-between px-6 md:px-12 py-5 md:py-[18px]">
        ...
        <Link to="/" className="relative z-10 flex items-center flex-shrink-0">
          <img
            ...
            style={{ filter: "invert(1) brightness(2)", opacity: scrolled ? 0.95 : 0.80, transition: "opacity 0.7s ease" }}
          />
        </Link>

        <nav className="hidden md:flex items-center" onMouseLeave={() => setActiveIndicator(null)}>
          ...
          <div
            ...
            style={{ left: activeIndicator?.left ?? 0, width: activeIndicator?.width ?? 0, background: "var(--leather)", opacity: activeIndicator ? 1 : 0 }}
          />
        </nav>
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            style={{ padding: "10px 8px", margin: "-10px -8px", background: "transparent", border: "none", cursor: "pointer", touchAction: "manipulation", WebkitTapHighlightColor: "transparent", zIndex: 60 }}
          >
      ...
      <div
        ...
        style={{
          ...
          borderTop: open ? "1px solid color-mix(in oklab, var(--leather) 18%, transparent)" : "none",
        ...
        <nav className="flex flex-col px-6 py-8 gap-0">
          {links.map((l, i) => (
            <Link
              ...
              style={{ borderColor: "color-mix(in oklab, var(--leather) 12%, transparent)", color: "rgba(255,255,255,0.5)", fontSize: "11px", textTransform: "uppercase", letterSpacing: "0.26em", transition: `color 0.2s ease ${i * 35}ms`, touchAction: "manipulation", WebkitTapHighlightColor: "transparent" }}
              activeProps={{ style: { color: "var(--leather)" } }}
            >
              <span>{l.label}</span>
              <span className="text-[10px] opacity-0 -translate-x-2 transition-all duration-300 group-hover:opacity-100 group-hover:translate-x-0" style={{ color: "var(--leather)" }}>→</span>
            </Link>
          ))}
          <ReserveButton mobile />
          <p className="mt-8 text-[10px] uppercase tracking-[0.24em] text-white/25">14 Goose Gate, Hockley · NG1 1FF</p>
        </nav>
      </div>
    </header>
  );
